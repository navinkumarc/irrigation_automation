// SerialConfigHandler.cpp  —  Serial config console implementation
#include "Config.h"
...
bool SerialConfigHandler::handle(const String &line) {
  ...
  if (up == "SAVE CONFIG")  { commCfg.save(_prefs); return true; }
  if (up == "RESET CONFIG") {
    Serial.println("[Config] Resetting to firmware defaults...");
    commCfg.reset(_prefs);
    commCfg.print();
    return true;
  }
...
bool SerialConfigHandler::handleEnable(const String &up, const String &) {
  // up already uppercase, e.g. "ENABLE SMS"
  String ch = up.substring(7);   // after "ENABLE "
  ch.trim();

  if      (ch == "SMS")                    { commCfg.chSMS       = true; Serial.println("[Config] SMS channel ENABLED"); }
  else if (ch == "DATA" || ch == "MQTT")   { commCfg.chData      = true; Serial.println("[Config] Data channel ENABLED"); }
  ...
  else if (ch == "SERIAL")                 { Serial.println("[Config] Serial is always ON — cannot be disabled"); }
...
bool SerialConfigHandler::handleDisable(const String &up, const String &) {
  String ch = up.substring(8);   // after "DISABLE "
  ch.trim();

  if      (ch == "SMS")                    { commCfg.chSMS       = false; Serial.println("[Config] SMS channel DISABLED"); }
  else if (ch == "DATA" || ch == "MQTT")   { commCfg.chData      = false; Serial.println("[Config] Data channel DISABLED"); }
  ...
  else if (ch == "SERIAL")                 { Serial.println("[Config] Serial cannot be disabled — it is the config console"); }
  else { Serial.println("[Config] Unknown channel: " + ch + "  (SMS | DATA | BLE | LORA)"); return true; }
...
bool SerialConfigHandler::handleSet(const String &up, const String &raw) {
  // up  = uppercase version, e.g.  "SET WIFI_SSID MYNET"
  // raw = original case,     e.g.  "SET wifi_ssid MyNet"
  String body = up.substring(4);   // after "SET "
  body.trim();
  int sp = body.indexOf(' ');
  if (sp <= 0) { Serial.println("[Config] Usage: SET <KEY> <value>"); return true; }

  String key = body.substring(0, sp);
  key.trim();

  // value preserves original case (for SSIDs, passwords, names, URLs)
